...
- Signature: canvas `htmlFor` association is inert (canvas not labelable; aria-label carries the name) — consider aria-labelledby. ~~Keyboard users cannot sign (inherent) — document in spec (Phase 7).~~ DONE in d4e0d43 (spec notes it). Optional-field garbage prefill shows the required message — consider a dedicated invalidSignature key. (Phase 6 review, Minors.)
