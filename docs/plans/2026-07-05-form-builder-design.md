...
6. Invisible fields (`visibleWhen` false) are unregistered from RHF (`shouldUnregister`) → values drop from output, validation skips automatically. No schema gymnastics.
