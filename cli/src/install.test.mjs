...
describe("resolveItemFiles", () => {
  ...
  it("resolves a field item to its source files with paths relative to form-builder/", () => {
    const files = resolveItemFiles("field-text", model);
  ...
  it("resolves a primitive item to components/ui/<name>.tsx, landing at components/ui/<name>.tsx", () => {
    const files = resolveItemFiles("fb-ui-button", model);
...
describe("installFormBuilder clobber protection", () => {
  ...
  it("writes every file on the first install (nothing pre-existing to skip)", () => {
    const summary = installFormBuilder({ mode: "add", fields: ["hidden"], cwd, install: false, theme: false });
    expect(summary.skipped).toEqual([]);
    expect(summary.written.length).toBeGreaterThan(0);
    expect(summary.fileCount).toBe(summary.written.length);
  });

  it("MAJOR fix: a second install SKIPS files that already exist, preserving a user's edit — unless --force", () => {
    installFormBuilder({ mode: "add", fields: ["hidden"], cwd, install: false, theme: false });
    ...
    const second = installFormBuilder({ mode: "add", fields: ["hidden"], cwd, install: false, theme: false });
    expect(second.skipped.length).toBeGreaterThan(0);
    expect(second.written).toEqual([]);
    expect(fs.readFileSync(targetFile, "utf8")).toBe(edited); // preserved, not clobbered

    const forced = installFormBuilder({ mode: "add", fields: ["hidden"], cwd, install: false, theme: false, force: true });
