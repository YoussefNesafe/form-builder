...
- `parseSubmission` (server-side validation, `core/parseSubmission.ts`) is synchronous and fails closed on `otp` with no skip flag — a missing `opts.otpVerified` on a visible otp field is always `code: "otp_checker_missing"`, never silently skipped. `file` fields and custom registered field types are always excluded from its schema (named in `unvalidated`, never fail-closed) — do not add a "validate files/custom types too" mode. `hidden` field values are re-injected from config unconditionally before visibility is computed — recursively into every `group` row, at any nesting depth, not just top-level fields; a per-row `hidden` value (price, SKU, line-item id) is exactly as protected as a top-level one. `disabled` field values are NOT specially handled and are trusted from the body as-is (server-owned values belong in `hidden`, never `disabled`). `maxStringLength` (default 10_000) is the size limit it owns for request content — a second, fixed, non-configurable structural depth cap (32) exists purely as self-protection, so a hostile deeply-nested body can't turn the size check's own recursion into a stack overflow; body size/rate limiting/group row counts still stay the host's job. A malformed config THROWS (not `ok:false`) — do not wrap it in a try/catch that downgrades it to a 400. See ADR-0004 and `/docs/server-validation` for the full rationale.
- `defineForm()` + the types-only `InferValues<C>`/`FieldNames<C>`/`FieldValue<F>` mapper (`form-builder/core/inferValues.ts`) are the type-inference seam — zero runtime. The generics threaded through `parseSubmission`/`FormRenderer`/`useDynamicForm`/`ServerErrorResult` are OPTIONAL and DEFAULTED (to `FormConfig`/`FormValues`/`string`) so untyped callers are unaffected — do NOT drop the defaults or collapse the generic back to the loose type. `InferValues` and the zod validation builder are two views of the same shape with nothing forcing agreement: when a field type's value changes, update BOTH (the `.test-d` design-table rows are the only guard). Payload optionality tracks conditional visibility (`visibleWhen`/`enabledWhenVerified`), NOT `required`; step-level `visibleWhen` is a known deferred soundness gap (type says required, runtime may strip) — documented on `/docs/type-safety`.
- New public symbols reachable from the HEADLESS surface (`defineForm`, `InferValues`, `FieldNames`, `FieldValue`, `DraftStorage`, `acceptedFormatsLabel`, ...) MUST be exported from BOTH `form-builder/index.ts` (app) and `form-builder/headless.ts` (the npm entry); a symbol missing from `headless.ts` ships nothing to npm consumers.
- The converse is NOT true: a symbol only a renderer can use — the components themselves, `ui/` primitives, and the types their props force a host to name (`StepperOrientation`, `DraftRestoreInfo`, `FileDropzoneProps`) — stays in `index.ts` alone. `headless.ts` ships no renderer to use it on, and a name a consumer cannot act on is contract surface bought for nothing. The test is neither directory nor type-vs-value: it is whether the symbol works without the engine's rendered tree (`useFieldRuntime` is a hook and is index-only, because it reads a context only `FormRenderer` populates; `formatReviewValue` lives in `components/` and is in both, because it is pure).
- Both barrels carry a header stating the above. `form-builder/barrels.test.ts` pins it over both barrels' source, covering TYPE placement; `form-builder/barrels.test-d.ts` pins the value surface through `@/form-builder` and `@/form-builder/headless` exactly as a consumer imports it (never a relative path). Both enumerate the index-only names rather than counting them. `createFormAction` lives ONLY under the `form-builder/next/` sub-path (`./next` in the package `exports` map), never the root barrel — keeps core framework-agnostic (no `next` import). Its source imports only `../core/*` (zero `next` import despite the folder name), so it IS shipped by the copy-in CLI: `form-builder/next/` is included in `ENGINE_DIRS` (`scripts/build-registry.mjs`) and lands as `app/form-builder/next/*` in a consumer install. Copy-in shipping it does NOT violate the "never the root barrel" rule (nothing is added to `index.ts`/`headless.ts`); the npm engine still gates it behind the `./next` export subpath so `import "<engine>"` never drags it in. A `build-registry.test.mjs` guard fails if any new top-level `form-builder/` dir is left unclassified by `ENGINE_DIRS`.
- Type-level regression is guarded by vitest `typecheck` mode (`vitest.config.ts` → `typecheck.include: ["**/*.test-d.{ts,tsx}"]`) plus scripted `yarn typecheck` (`tsc --noEmit`) in CI. `.test-d.ts` files are NO-OPS as plain runtime tests — their `expectTypeOf` assertions run ONLY under typecheck — so do NOT disable typecheck mode or rename these to `.test.ts`.
