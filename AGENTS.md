...
- `parseSubmission` (server-side validation, `core/parseSubmission.ts`) is synchronous and fails closed on `otp` with no skip flag — a missing `opts.otpVerified` on a visible otp field is always `code: "otp_checker_missing"`, never silently skipped. `file` fields and custom registered field types are always excluded from its schema (named in `unvalidated`, never fail-closed) — do not add a "validate files/custom types too" mode. `hidden` field values are re-injected from config unconditionally before visibility is computed; `disabled` field values are NOT specially handled and are trusted from the body as-is (server-owned values belong in `hidden`, never `disabled`). `maxStringLength` (default 10_000) is the only size limit it owns — body size/rate limiting/group row counts stay the host's job. A malformed config THROWS (not `ok:false`) — do not wrap it in a try/catch that downgrades it to a 400. See ADR-0004 and `/docs/server-validation` for the full rationale.
