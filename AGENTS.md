...
- New public symbols reachable from the HEADLESS surface (`defineForm`, `InferValues`, `FieldNames`, `FieldValue`, `DraftStorage`, `acceptedFormatsLabel`, ...) MUST be exported from BOTH `form-builder/index.ts` (app) and `form-builder/headless.ts` (the npm entry); a symbol missing from `headless.ts` ships nothing to npm consumers. The converse is NOT true: a symbol only a renderer can use — the components themselves, `ui/` primitives, and the types their props force a host to name (`StepperOrientation`, `DraftRestoreInfo`, `FileDropzoneProps`) — stays in `index.ts` alone, because `headless.ts` ships no renderer to use it on and a name a consumer cannot act on is contract surface bought for nothing. Both barrels carry a header stating this; `form-builder/barrels.test-d.ts` pins it, importing through `@/form-builder` and `@/form-builder/headless` exactly as a consumer does (never a relative path) and enumerating — not counting — every index-only value. `createFormAction` lives ONLY under the `form-builder/next/` sub-path (`./next` in the package `exports` map), never the root barrel — keeps core framework-agnostic (no `next` import). Its source imports only `../core/*` (zero `next` import despite the folder name), so it IS shipped by the copy-in CLI: `form-builder/next/` is included in `ENGINE_DIRS` (`scripts/build-registry.mjs`) and lands as `app/form-builder/next/*` in a consumer install. Copy-in shipping it does NOT violate the "never the root barrel" rule (nothing is added to `index.ts`/`headless.ts`); the npm engine still gates it behind the `./next` export subpath so `import "<engine>"` never drags it in. A `build-registry.test.mjs` guard fails if any new top-level `form-builder/` dir is left unclassified by `ENGINE_DIRS`.
